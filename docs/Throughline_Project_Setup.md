...
**Done since (forgot-password, `src/auth`):** `requestPasswordReset(email)` and `updatePassword(newPassword)` added to the auth module; `/forgot-password` and `/reset-password` pages built. `/auth/confirm` reused as-is - it was already generic over `code`/`token_hash` and email `type`, so no route logic changed, just its failure redirect now branches on the intended `next` destination (`/forgot-password?error=link_failed` vs `/sign-in?error=verification_failed`) so a failed recovery link doesn't show a sign-up-specific message.

**Real bug found via live testing, not a code defect - a PKCE/email-link design gap:** submitting the reset form from one browser context and clicking the emailed link from a different one fails with Supabase's own `bad_code_verifier` error (confirmed via `query_logs(source: auth_logs)` - GoTrue's PKCE flow ties the flow to a `code_verifier` cookie local to whichever browser initiated it). This is a real production concern, not just a testing artifact: email links are routinely opened in a different browser/app than the one that requested them. Sign-up verification happened to survive an equivalent mismatch earlier only because Supabase confirms the email at its own hosted page regardless of the follow-up exchange; password recovery has no such fallback - the exchange **is** the whole flow.

**Fix (confirmed with the user):** switch both email templates from `{{ .ConfirmationURL }}` to the `token_hash` pattern, which needs **zero code changes** - `verifyEmailOtp` and the generic `/auth/confirm` route were already built to handle this path. `verifyOtp` verifies server-side with no local browser state, so it isn't sensitive to which browser clicks the link. No MCP tool exposes Auth email template or SMTP configuration (confirmed via `ToolSearch`) - both were manual dashboard steps for the user.

**Two more real things found only by testing live, not by reasoning about the templates:**
- Supabase gates raw template-source editing behind having **custom SMTP** configured (to stop abuse of its shared default sender) - not anticipated going in. Resolved with Resend, using its no-verification-needed `onboarding@resend.dev` testing sender - no domain purchase/DNS work needed for an 8-day capstone.
- `{{ .Type }}` did not render in this template engine - came through as a literal empty string (`&type=`), confirmed by inspecting the actual emailed link, which made `/auth/confirm`'s `tokenHash && type` check fail **silently with zero calls to Supabase at all** (the diagnostic tell: no `/verify` log entry whatsoever after the switch, until this was found). Fixed by hardcoding `type=recovery` / `type=signup` per template instead of depending on the variable - each template is always exactly one type anyway.

**Verified end-to-end against the live project**, not just the UI's success message: `query_logs(source: auth_logs)` shows the complete clean chain - `user_recovery_requested` -> `/verify` succeeding with `login_method: "otp"` (confirming the token_hash path, not PKCE) -> `PUT /user` (`user_modified`) -> sign-in with the **new** password succeeding -> a sign-in attempt with the **old** password correctly failing (`invalid_credentials`). Recorded as ERD Appendix B round 11 (v1.7 -> v1.8).
