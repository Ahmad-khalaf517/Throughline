...
**Fix (confirmed with the user):** switch both email templates from `{{ .ConfirmationURL }}` to the `token_hash` pattern (`{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type={{ .Type }}`, `&next=/reset-password` on the reset template), which needs **zero code changes** - `verifyEmailOtp` and the generic `/auth/confirm` route were already built to handle this path (it was already documented as the primary path for a customized template). `verifyOtp` verifies server-side with no local browser state, so it isn't sensitive to which browser clicks the link. No MCP tool exposes Auth email template configuration (confirmed via `ToolSearch`) - this is a manual dashboard step (Authentication > Email Templates) the user needs to make; not yet re-tested end-to-end pending that change.
