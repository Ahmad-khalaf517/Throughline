...
1. **Plan.** Read the Jira Plan row and the cited sections of its source documents. Search by identifier or heading and open only the needed ranges; expand when a citation or contradiction requires more context. Load `throughline-doc-sync` only for documentation, schema, or identifier changes, then state module ownership, files, cited T##/FR/INV/API route coverage, dependencies, and scope exclusions. The frozen source documents take precedence over code; report contradictions and stop.
...
- Use the project default Sol model for routine coordination; select Sol when starting a feature task if the user-wide default is Astra. Keep explicit role models: Sol for implementation and boundary review, Luna for verification/citations/document consistency, Astra for invariant review. Do not escalate a whole workflow merely because one specialist needs deeper reasoning.
- Prefer a fresh Codex task for each new ticket, with its own worktree and compact story packet. When several tickets are explicitly requested in one task, process them serially and carry forward only the per-story record and dependency facts needed for the next ticket; do not create a separate task without the user's explicit request.
- For feature work, keep Atlassian available. Enable Stitch only while a UI story needs it. Other connectors are task-specific; keep them disabled when unused.
