...
4. Read the story's row in `docs/Throughline_Jira_Plan.md` and every document section it **Cites**. The four parent documents are frozen: if the code you are about to write disagrees with them, the code is wrong, not the document. Stop and say so rather than "fixing" the document.
