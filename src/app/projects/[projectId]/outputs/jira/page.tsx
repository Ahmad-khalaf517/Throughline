...
/**
 * `GET/POST /api/projects/:projectId/jira/{preview,export}` screen (E5-S9,
 * API Contracts section 9). Auth guard / `notFound()` mapping copied from
 * `warnings/page.tsx` exactly - all the real data fetching and the write
 * itself happen client-side in `JiraExportPanel` (Module Boundaries 4.8).
 */
export default async function ProjectJiraOutputPage({ params }: ProjectJiraOutputPageProps) {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-3xl flex-col gap-6 px-4 py-12 sm:px-6">
      ...
      <header className="border-surface-dim bg-surface-container-lowest rounded-xl border p-6">
        <h1 className="text-on-surface text-display-sm font-semibold">Jira</h1>
        <p className="text-on-surface-variant mt-1 text-sm">
          Export {project.name}&apos;s approved Backlog as Epics and Stories.
        </p>
      </header>
